---
post_id: 74909d58-c1f3-54eb-9523-4de491b0ba9c
title: 멀티테넌시? 어떻게 설계하고 접근하지?
description: "Shared Database/Shared Schema부터 Database per Tenant까지 멀티테넌시 아키텍처 패턴들의 트레이드오프와 선택 기준을 정리했습니다."
...
멀티테넌시에서 **테넌트**는 "데이터와 설정이 서로 격리되어야 하는 독립적인 논리적 단위"를 의미합니다. 보통 하나의 **고객 회사(Organization), 기관, 또는 과금/계약 단위**를 테넌트로 봅니다.

### 테넌트 여부 판단 체크리스트

1. **데이터 격리가 필수적인가?**: 해당 단위의 데이터가 타 단위와 완전히 분리되어야 한다면 테넌트입니다. 내부 공유 목적이라면 테넌트 내부의 '그룹/팀'으로 처리합니다.
2. **과금 및 관리의 주체인가?**: 독립된 요금 청구, 관리자 계정, 설정 값이 필요하다면 테넌트로 정의합니다.
3. **보안 및 규제 준수 대상인가?**: 법적·보안상 이유로 데이터 혼용이 금지된 단위라면 테넌트입니다.
...
결론적으로, **대부분의 SaaS에서는 조직/기관/회사 = 테넌트**이며, 내부의 그룹이나 팀은 테넌트 내의 하위 구조로 설계하는 것이 일반적입니다.
