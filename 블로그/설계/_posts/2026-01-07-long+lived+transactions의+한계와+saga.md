...
LLT는 일반적인 ACID 트랜잭션이 **수초를 넘어 수분, 심지어 며칠**까지 지속되는 경우를 말합니다.

단기 트랜잭션은 즉시 종료되지만, LLT는 아래와 같은 상황에서 필연적으로 발생합니다:

- **주문 처리 흐름**  
  상품 선택 → 옵션 구성 → 할인 적용 → 결제 입력 → 최종 확인  
  → 사용자 대기만으로 10분 이상 소요

- **회원 가입/인증**  
  기본 정보 → 본인 인증 → 약관 동의 → 추가 정보 입력  
  → 중간 이탈/재개로 세션 길어짐

- **내부 승인 워크플로**  
  신청 → 1차 검토 → 2차 검토 → 최종 승인  
  → 여러 담당자 입력으로 하루 이상 소요

이런 흐름은 비즈니스적으로 **원자성**이 필요하지만, DBMS는 장기 Lock을 감당할 수 없습니다.
...
Saga 패턴에서는 각 로컬 트랜잭션이 자신의 작업을 마칠 때마다 데이터베이스에 곧바로 커밋합니다.  
이 과정에서 전체 비즈니스 프로세스가 완전히 끝나지 않았음에도 불구하고,  
개별 단계(예: 재고 차감, 금액 이체 등)의 결과가 데이터베이스에 반영되어  
다른 트랜잭션 혹은 외부 시스템에 '중간 상태(intermediate state)'로 노출될 수 있습니다.  
즉, 한 Saga 인스턴스가 작업을 단계별로 완료해가는 동안,  
아직 전체 Saga가 최종 성공 또는 보상 트랜잭션으로 롤백하기 전이라도 이미 수행된 일부 변경사항이  
다른 사용자의 트랜잭션, 혹은 동시에 진행 중인 다른 Saga 인스턴스에 그대로 보여질 수 있다는 의미입니다.  
이런 식의 중간 상태 노출은 전통적인 ACID 트랜잭션(특히 Isolation의 보장)과 달리,  
업데이트의 일관적 완결이 보장되지 않는 문제를 발생시키고,  
그로 인해 잘못된 근거로 의사결정에 들어가거나, 논리적 오류를 야기할 수 있는 위험이 존재합니다.

**문제 예시**  
Saga A가 재고 차감 후 결제 대기 중 → Saga B가 차감된 재고 조회 → 잘못된 결정 → Saga A 실패 후 재고 복구 → Saga B의 결정 무효화

### 주요 해결 기법

1. **Semantic Lock**  
   데이터에 "처리 중" 상태 필드 추가 → 다른 Saga가 상태 확인 후 진행

2. **낙관적 락**  
   Version 컬럼으로 충돌 감지 → 변경 시 버전 확인

3. **Commutative Update**  
   순서 무관 연산 설계 (예: quantity = quantity - 10)

4. **비즈니스 수준 검증**  
   보상 전 다른 Saga 영향 확인 → 필요 시 연쇄 보상

**실제 사례**  
포인트 차감 Saga 중 조회 시 잘못된 잔액 노출 → Semantic Lock 도입 후 해결
...
복잡한 도메인 로직과 정책, 그리고 대량의 insert 작업으로 인해 하나의 DB 트랜잭션이 너무 길어지는 상황에 직면했습니다. 처리 속도가 충분히 빠른 경우에는 단일 트랜잭션으로 묶는 것이 가장 이상적이지만, 요청 처리 시간이 분 단위로 늘어나면서 대량의 요청이 몰릴 때 여러 가지 문제가 발생하곤 했습니다. 이러한 문제를 해결하기 위해 자료를 찾다가 sagas 논문을 알게 되었고, 공부한 내용을 정리한 이 글이 비슷한 고민을 하시는 분들께 도움이 되었으면 합니다.