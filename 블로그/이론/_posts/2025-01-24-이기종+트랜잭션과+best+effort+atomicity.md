...
| ---- | ---------------- | -------------------------------------------------- |
| 1    | Kafka 전송 실패      | **Outbox 테이블**에 이벤트 저장 → 별도 프로세스가 재전송 보장           |
| 2    | DB 실패 후 Kafka 성공 | **DB 커밋 완료 후에만 Kafka 전송** → DB 실패 시 전송 자체를 시도하지 않음 |
...
| ---- | ------------------------------------------------------ | ------------------------- |
| 1    | Outbox 테이블에 이벤트 저장 → 릴레이 프로세스가 재시도하여 Kafka 전송 보장       | **Eventually Consistent** |
| 2    | DB 커밋 실패 → Outbox 테이블에 이벤트 기록되지 않음 → Kafka 전송 시도 자체 없음 | **일관성 유지**                |
